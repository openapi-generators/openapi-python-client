...
1. Pydantic ``X | None`` field typing instead of upstream's ``X | Unset`` — the
   ``test_type_hints`` assertions.

2. Pydantic validation/serialization semantics — strict ``from_dict`` raises
...
# Bucket 1 — optional fields are `X | None`, not `X | Unset`.
...
# Bucket 2 — Pydantic validation/serialization differs from attrs.
...
def pytest_collection_modifyitems(config, items):
    for item in items:
        nodeid = item.nodeid
        if any(key in nodeid for key in _XFAIL_TYPE_HINT_CONTRACT):
